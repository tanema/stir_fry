...
class ResponseSpecComponent < StirFry::Component
  template_source "<p>a component</p>"
end

RSpec.describe StirFry::Response do
  ...
  describe "bang methods" do
    it "raise their matching StirFry error with the given message" do
      resp = build_response
      expect { resp.not_found!("gone") }.to raise_error(StirFry::NotFound, "gone")
      expect { resp.bad_request!("nope") }.to raise_error(StirFry::BadRequest, "nope")
      expect { resp.unauthorized!("no") }.to raise_error(StirFry::Unauthorized, "no")
      expect { resp.internal_error!("oops") }.to raise_error(StirFry::InternalError, "oops")
      expect { resp.ok!("done") }.to raise_error(StirFry::OKAY, "done")
    end
  end
  ...
  describe "#etag" do
    ...
    it "raises NotModified for a safe request whose If-None-Match matches" do
      ...
      expect { resp.etag("abc") }.to raise_error(StirFry::NotModified)
    end
    ...
    it "raises PreconditionFailed for an unsafe request whose If-None-Match matches" do
      ...
      expect { resp.etag("abc") }.to raise_error(StirFry::PreconditionFailed)
    end
  ...
  describe "#last_modified conditional handling" do
    ...
    it "raises NotModified when If-Modified-Since is at or after the given time" do
      ...
      expect { resp.last_modified(time) }.to raise_error(StirFry::NotModified)
    end
    ...
    it "raises PreconditionFailed when If-Unmodified-Since predates the given time" do
      ...
      expect { resp.last_modified(time) }.to raise_error(StirFry::PreconditionFailed)
    end
  ...
  describe "file serving" do
    ...
    it "serves the full file with a last-modified header and correct length" do
      ...
      req = StirFry::Request.new(Rack::MockRequest.env_for("/hello.txt"))
    ...
    it "serves a byte range with 206, a content-range header, and the range's length" do
      req = StirFry::Request.new(Rack::MockRequest.env_for("/hello.txt", "HTTP_RANGE" => "bytes=0-4"))
    ...
    it "returns 416 for an unsatisfiable range" do
      req = StirFry::Request.new(Rack::MockRequest.env_for("/hello.txt", "HTTP_RANGE" => "bytes=9999-10000"))
    ...
    it "raises NotFound for a missing file" do
      req = StirFry::Request.new(Rack::MockRequest.env_for("/nope.txt"))
      resp = build_response("/nope.txt")
      expect { resp.send_file(req, "/no/such/file.txt") }.to raise_error(StirFry::NotFound)
    end
