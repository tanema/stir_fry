...
module AppSpecFixtures
  ...
  class BlockingMiddleware
    ...
    def call(_env) = raise StirFry::Unauthorized
  end
end

class AppSpecApp < StirFry::App
  def self.trace
    @trace ||= []
  end
  ...
  scope do
    use AppSpecFixtures::HeaderMiddleware, "X-Powered-By", "stir_fry"
...
class OtherAppSpecApp < StirFry::App
  get("/only-here") { |response:, **| response.text("isolated-ok") }
end

RSpec.describe StirFry::App do
  ...
  it "runs route-level middleware around the handler" do
    ...
    expect(resp.headers["X-Powered-By"]).to eq("stir_fry")
  end
